...
FILE_PATH = "./data/2022/day_2.txt"


def day_2(file_path: str = FILE_PATH) -> int:
    ...
    rounds = [x.lower().replace("\n", "").split(" ") for x in rounds]
    ...
    for x, y in rounds:
        ...
        opponent = score_map["opponent"][x]
        player = score_map["player"][y]

        scores.append(_score(opponent, player))
        new_scores.append(_second_strategy(opponent, player))

    return sum(scores), sum(new_scores)


def _score(opponent: int, player: int) -> int:

    score_delta = player - opponent

    modifier = 0

    if score_delta > 0 and score_delta != 2 or score_delta == -2:
        modifier = 6
    elif score_delta == 0:
        modifier = 3
...
def _second_strategy(opponent: int, player: int) -> int:
