...
- The first rucksack contains the items vJrwpWtwJgWrhcsFMMfFFhFp, which
    ...
    the second compartment contains the items hcsFMMfFFhFp. The only
...
FILE_PATH = "./data/2022/day_3.txt"


def day_3(file_path: str = FILE_PATH) -> int:
    ...
    for rucksack in rucksacks:
        ...
        if len(common_item) > 1:
            raise ValueError("Too much common items")

        common_item = common_item.pop()

        priority = ord(common_item) - ord('a') + 1

        # add diff between 'A' and 'a' + 27 to start at new range
        if priority < 0:
            priority += 32 + 26
